...
void main() {
  ...
  ///全局初始化
  Global.init().then((value) => runApp(const ApplicationWidget()));
}
...
class MyAppState extends State<MyApp> with LoggerPlus, WidgetsBindingObserver {
  double ratio = 0;

  @override
  Widget build(BuildContext context) {
    var defaultTransition = Platform.isIOS ? null : Transition.cupertino;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    log('didChangeAppLifecycleState. state=$state this=$hashCode');
    // Store.instance.onAppStateChange(state);
    // Services.instance.onAppStateChange(state);
  }
}
